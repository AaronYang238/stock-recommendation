...
**用户 2026-08-16 决定:两个基线都跑对照**——① 涨停即清(首次涨停次日开盘全清,否则 20 日/换仓日清);② 固定盈利 X% 即清(默认 +8%)。两者都与吊灯移动止损纪律对照,`run_exit_ablation` 返回三组(trailing / sell_on_limit / fixed_pct)+ 各自盈亏比 delta。
