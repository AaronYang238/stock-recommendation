...
> 若服务器出口 IP 被东方财富封锁（akshare 行情连接被 reset），改用 **tushare**：注册拿
> token → `export TUSHARE_TOKEN=...`，并把 `config/config.yaml` 的 `datasource.primary`
> 设为 `tushare`。tushare 走 API、一般不被封，且能提供 ROE/毛利率/同比等真实财务与**披露日**
> （PIT 回测所需）。akshare 仍可单独用于行业映射。
